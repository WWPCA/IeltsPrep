#!/usr/bin/env python3
"""
Deploy comprehensive privacy policy and terms templates matching the local preview
"""
import boto3
import zipfile
import base64

def deploy_comprehensive_privacy_template():
    """Deploy the exact privacy policy and terms templates from local preview"""
    ...
    # Verify templates
    if '/privacy-policy' not in login_template:
...
<body>
    ...
    # Create Lambda function with comprehensive templates matching local preview
    lambda_code = f'''
import json
import base64

def lambda_handler(event, context):
    """Lambda handler with comprehensive templates matching local preview"""
    path = event.get('path', '/')
...
'''
    ...
    with zipfile.ZipFile('comprehensive-privacy-templates.zip', 'w', zipfile.ZIP_DEFLATED) as zip_file:
        zip_file.writestr('lambda_function.py', lambda_code)
    
    # Deploy to AWS Lambda
    lambda_client = boto3.client('lambda', region_name='us-east-1')
    
    with open('comprehensive-privacy-templates.zip', 'rb') as f:
        zip_content = f.read()
    
    print('Deploying comprehensive privacy policy and terms templates...')
...
if __name__ == "__main__":
    deploy_comprehensive_privacy_template()