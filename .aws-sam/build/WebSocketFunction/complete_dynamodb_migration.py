...
def populate_questions_quickly():
    """Quickly populate the questions table"""
    try:
        dynamodb = boto3.resource('dynamodb', region_name='us-east-1')
        table = dynamodb.Table('ielts-assessment-questions')
        
        # Sample questions for each type (can be expanded later)
        questions_data = {
            "academic-writing": [
        ...
        questions_count = 0
        for assessment_type, questions in questions_data.items():
            for question in questions:
                item = {
                    'assessment_type': assessment_type,
                    'question_id': question['id'],
                    'question_data': question,
                    'created_at': '2025-07-08T10:50:00Z',
                    'active': True
                }
        ...
        print(f"❌ Error populating: {str(e)}")
        return False

def create_dynamodb_lambda_code():
    """Create Lambda code that uses DynamoDB for questions"""
    
    # Read current working template
...
def get_questions_from_dynamodb(assessment_type: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        fallback_questions = {
            "academic-writing": [
                {"id": "aw_fallback_001", "task_type": "Task 2 - Essay", "prompt": "Some people believe that technology has made our lives easier, while others think it has made them more complicated. Discuss both views and give your opinion.", "time_limit": 40, "word_count_min": 250}
            ],
            "academic-speaking": [
                {"id": "as_fallback_001", "parts": [{"part": 1, "topic": "Hobbies", "questions": ["What hobbies do you have?", "How long have you had these hobbies?", "Why do you enjoy them?"]}, {"part": 2, "topic": "Describe a hobby you enjoy", "prep_time": 60, "talk_time": 120}, {"part": 3, "topic": "Leisure Activities", "questions": ["How have leisure activities changed?", "What benefits do hobbies provide?"]}]}
            ],
            "general-writing": [
                {"id": "gw_fallback_001", "task_type": "Task 1 - Letter", "prompt": "You recently attended a course but were not satisfied. Write a letter to the course provider. In your letter: explain what course you attended, describe what you were dissatisfied with, say what action you would like them to take.", "time_limit": 20, "word_count_min": 150, "letter_type": "Complaint"}
            ],
            "general-speaking": [
                {"id": "gs_fallback_001", "parts": [{"part": 1, "topic": "Daily Routine", "questions": ["What is your daily routine?", "What do you like about your routine?", "What would you change?"]}, {"part": 2, "topic": "Describe a typical day in your life", "prep_time": 60, "talk_time": 120}, {"part": 3, "topic": "Work-Life Balance", "questions": ["How important is work-life balance?", "How do people manage their time?"]}]}
            ]
        }
        return fallback_questions.get(assessment_type, [])
...
def get_unique_assessment_question(user_email: str, assessment_type: str) -> Optional[Dict[str, Any]]:
    ...
    if not all_questions:
        print(f"No questions found for {assessment_type}")
        return None
    ...
    if available_questions:
        ...
        # All questions used - reset for new cycle
...
def lambda_handler(event, context):
    """Main AWS Lambda handler with DynamoDB question management"""
    try:
        path = event.get("path", "/")
...
<body>
    ...
    <div class="container py-5" style="margin-top: 76px;">
        ...
        <div class="alert alert-info">
            ...
            <p class="mb-0">
                Questions are now dynamically loaded from DynamoDB, enabling easy question bank expansion and management.
            </p>
        </div>
            ...
            if not question:
                return {
                    ...
                    "body": """<html><body style="text-align: center; padding: 50px;">
                        ...
                        <p>Unable to load assessment question from DynamoDB.</p>
                        <a href="/dashboard">Back to Dashboard</a>
                    </body></html>"""
                }
...
<body>
    ...
    <script>
        ...
        // Timer countdown
        let timeLeft = {question.get('time_limit', 40)} * 60;
...
<body>
    ...
    <script>
        ...
        console.log('WebSocket URL:', '{maya_session.get("websocket_url", "N/A")}');
            ...
            # User profile page (existing code preserved)
            cookie_header = headers.get("Cookie", "")
            session_id = None
            for cookie in cookie_header.split(";"):
                if "session_id=" in cookie:
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header bg-info text-white">
                        ...
                        <p class="mb-0">Questions powered by DynamoDB</p>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <p><strong>Question Source:</strong> DynamoDB</p>
                            </div>
                            <div class="col-md-6">
            ...
            <div class="col-lg-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <p>Assessment questions are stored in AWS DynamoDB to ensure unique question delivery and prevent repetition across multiple purchases.</p>
...
def deploy_dynamodb_lambda():
    """Deploy updated Lambda function with DynamoDB integration"""
    try:
        # Populate questions first
        if not populate_questions_quickly():
            print("❌ Failed to populate questions")
            return False
        
        # Create Lambda code
        lambda_code = create_dynamodb_lambda_code()
        ...
        print('')
        print('🎯 DYNAMODB INTEGRATION DEPLOYED:')
        print('  ✅ Questions moved from hardcoded arrays to DynamoDB table')
        print('  ✅ get_questions_from_dynamodb() function retrieves questions dynamically')
        print('  ✅ Fallback questions included for reliability')
        print('  ✅ All assessment pages now show "Question ID: xxx (from DynamoDB)"')
        print('  ✅ Dashboard shows "DynamoDB Question System Active"')
        print('  ✅ Profile page displays "Question Source: DynamoDB"')
        print('')
        print('💡 BENEFITS:')
        ...
        print('   All assessments now use DynamoDB for question delivery!')
...
if __name__ == "__main__":
    print("🚀 COMPLETING DYNAMODB MIGRATION")
    print("=" * 50)
    deploy_dynamodb_lambda()
