#!/usr/bin/env python3
"""
Deploy new header text to production
"""
import boto3
import zipfile
import base64

def deploy_new_header_text():
    """Deploy the new header text to AWS Lambda production"""
    
    # Read the updated home template with new header text
    ...
    # Verify the new header text is present
    if 'Master IELTS Writing & Speaking with GenAI Precision' not in home_template:
        print("ERROR: New header text not found in template")
        return False
    
    if 'Unlock your full potential' not in home_template:
        print("ERROR: New description text not found in template")
        return False
...
<body>
    ...
    # Create Lambda function with new header text
    lambda_code = f'''
import json
import base64

def lambda_handler(event, context):
    """Lambda handler with new header text"""
    path = event.get('path', '/')
...
def serve_home_page():
    """Serve home page with new header text"""
...
'''
    ...
    with zipfile.ZipFile('new-header-text.zip', 'w', zipfile.ZIP_DEFLATED) as zip_file:
        zip_file.writestr('lambda_function.py', lambda_code)
    
    # Deploy to AWS Lambda
    lambda_client = boto3.client('lambda', region_name='us-east-1')
    
    with open('new-header-text.zip', 'rb') as f:
        zip_content = f.read()
    
    print('Deploying new header text to production...')
