#!/usr/bin/env python3
"""
Deploy Lambda with login page that uses working API endpoint
"""
import boto3
import zipfile
import os
...
def create_working_login_lambda():
    """Create Lambda with login that uses direct API Gateway endpoint"""
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        loginForm.addEventListener('submit', async function(e) {
            ...
            try {
                // Use direct API Gateway endpoint that works
                const response = await fetch('https://n0cpf1rmvc.execute-api.us-east-1.amazonaws.com/prod/api/login', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
...
<body>
    ...
    with zipfile.ZipFile('lambda-working-login.zip', 'w', zipfile.ZIP_DEFLATED) as zip_file:
        zip_file.writestr('lambda_function.py', lambda_code)
    
    return 'lambda-working-login.zip'

def deploy_working_login():
    """Deploy Lambda with working login"""
