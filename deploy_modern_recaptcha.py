#!/usr/bin/env python3
"""
Deploy modern reCAPTCHA integration using "On User Interaction" approach
Implements Google's enterprise reCAPTCHA with grecaptcha.execute() on user interaction
"""
...
def create_lambda_with_modern_recaptcha():
    """Create Lambda function with modern reCAPTCHA integration"""
    ...
    # Create Lambda function code with modern reCAPTCHA
    lambda_code = f'''
"""
AWS Lambda Handler for IELTS GenAI Prep with Modern reCAPTCHA Integration
Uses "On User Interaction" approach with grecaptcha.execute()
"""
...
def lambda_handler(event, context):
    """Main AWS Lambda handler with modern reCAPTCHA"""
...
def handle_home_page() -> Dict[str, Any]:
    ...
    # Template with corrected content
    template = """{template}"""
...
def handle_login_page() -> Dict[str, Any]:
    """Handle login page with modern reCAPTCHA integration"""
    ...
    <body>
        ...
        <div class="login-container">
            ...
            <form id="loginForm" method="POST" action="/login">
                ...
                <button type="button" id="loginBtn" class="btn btn-primary w-100 mb-3 btn-login">
...
def handle_user_login(data: Dict[str, Any], user_ip: Optional[str] = None) -> Dict[str, Any]:
    """Handle user login with modern reCAPTCHA verification"""
    ...
    if not verify_recaptcha_v2(recaptcha_response, user_ip):
        return {{
            ...
            'body': '<html><body><h2>Security Verification Failed</h2><p>Please complete the security verification and try again.</p><a href="/login">Try Again</a></body></html>'
        }}
...
def deploy_modern_recaptcha():
    """Deploy Lambda function with modern reCAPTCHA integration"""
    ...
    try:
        ...
        # Update environment variables with reCAPTCHA keys
        try:
            ...
            print("✓ Environment variables updated with reCAPTCHA keys")
        except Exception as e:
            print(f"Note: Could not update environment variables: {str(e)}")
        
        print("✓ Modern reCAPTCHA integration deployed successfully!")
        print(f"Function ARN: {response['FunctionArn']}")
        print(f"Last Modified: {response['LastModified']}")
        print(f"Version: {response['Version']}")
        print(f"Production URL: https://www.ieltsaiprep.com")
        print("\n🔧 Modern reCAPTCHA Features:")
        print("  • 'On User Interaction' approach - executes when user clicks login")
        print("  • Uses Google's enterprise reCAPTCHA API")
        print("  • Real-time token generation and verification")
        print("  • Seamless user experience with loading states")
        print("  • Proper error handling and user feedback")
        print("  • Server-side verification with Google's API")
        print("  • Action-based scoring (LOGIN action)")
        print("  • 2-minute token expiration for security")
