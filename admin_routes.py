...
@admin_bp.route('/tax-verification')
@login_required
@admin_required
def tax_verification():
    """Display the tax verification tool."""
    ...
    return render_template(
        'admin/tax_verification.html',
        title='Tax Verification Tool',
        results=results
    )

@admin_bp.route('/run-tax-verification', methods=['POST'])
@login_required
@admin_required
def run_tax_verification():
    """Run tax verification tests and display results."""
    try:
        # Import the tax verification module
        from tax_verification_script import run_tax_tests
        
        # Run the tests
        results = run_tax_tests()
        
        # Convert the results to a JSON string for passing via URL
        results_json = json.dumps(results)
        
        # Redirect to the tax verification page with results
        flash('Tax verification tests completed successfully.', 'success')
        return redirect(url_for('admin.tax_verification', results=results_json))
    except Exception as e:
        flash(f'Error running tax verification tests: {str(e)}', 'danger')
        return redirect(url_for('admin.tax_verification'))