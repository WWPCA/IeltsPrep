#!/usr/bin/env python3
"""
Deploy updated template with Legal button removed from header
Keeps privacy policy and terms of service links in footer only
"""
import boto3
import zipfile
import base64

def deploy_updated_template_no_legal():
    """Deploy updated template without Legal dropdown in header"""
    
    # Read the updated home template (Legal button removed)
    ...
    # Verify Legal button was removed
    if 'Legal' in home_template:
        print("ERROR: Legal button still present in home template")
        return False
    
    # Verify footer links are still present
    if '/privacy-policy' not in home_template or '/terms-of-service' not in home_template:
        print("ERROR: Footer privacy links missing from home template")
        return False
    ...
    print(f"Template verified: Legal button removed, footer links present, {pricing_count} instances of $49.99 pricing")
    ...
    # Create comprehensive privacy policy matching local preview
...
<body>
    ...
    # Create comprehensive terms of service matching local preview
...
<body>
    ...
    # Create Lambda function with all updated templates
    lambda_code = f'''
import json
import base64

def lambda_handler(event, context):
    """Lambda handler with updated templates - no Legal button in header"""
    path = event.get('path', '/')
...
def serve_home_page():
    """Serve home page with Legal button removed from header"""
...
def serve_privacy_policy():
    """Serve comprehensive privacy policy matching local preview"""
...
def serve_terms_of_service():
    """Serve comprehensive terms of service matching local preview"""
...
'''
    ...
    with zipfile.ZipFile('updated-template-no-legal.zip', 'w', zipfile.ZIP_DEFLATED) as zip_file:
        zip_file.writestr('lambda_function.py', lambda_code)
    
    # Deploy to AWS Lambda
    lambda_client = boto3.client('lambda', region_name='us-east-1')
    
    with open('updated-template-no-legal.zip', 'rb') as f:
        zip_content = f.read()
    
    print('Deploying updated template without Legal button...')
...
if __name__ == "__main__":
    deploy_updated_template_no_legal()