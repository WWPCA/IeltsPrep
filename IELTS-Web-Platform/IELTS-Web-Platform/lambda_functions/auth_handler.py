...
def handle_account_deletion(event):
    ...
    try:
        ...
        try:
            ...
            # Scan and delete all user assessments
...
def handle_reset_password(body):
    ...
    try:
        ...
        try:
            ...
            print(f"Error invalidating existing tokens: {str(e)}")
            # Don't fail the request if this fails
...
def create_reset_tokens_table():
    """
    Create the reset tokens table if it doesn't exist
    """
    try:
        table = dynamodb.create_table(
            TableName='ielts-genai-prep-reset-tokens',
            KeySchema=[
                {
        ...
        print("Reset tokens table created successfully")
        
    except ClientError as e:
        if e.response['Error']['Code'] != 'ResourceInUseException':
            print(f"Error creating reset tokens table: {str(e)}")
            raise e
